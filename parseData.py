...
# Lê os csv's e a função lambda converte valores para lowercase.
...
# Converte coluna 'Quantidade de ingressantes no curso' para int
fluxo['Quantidade de Ingressantes no Curso'] = fluxo['Quantidade de Ingressantes no Curso'].str.replace(',', '').astype(int)

# Agrupa por universidade e curso, soma a quantia de ingressantes e pega a primeira taxa de desistência acumulada.
...
dfFinal = pd.merge(dfFinal, fluxo[['Nome da Instituição', 'Código do Curso de Graduação', 'Nome do Curso de Graduação',
                                   'Quantidade de Ingressantes no Curso',
...
dfFinal = dfFinal[['Nome da Instituição', 'Código do Curso de Graduação', 'Nome do Curso de Graduação',
                   'Quantia de Bolsas', 'Quantidade de Ingressantes no Curso', 'Percentual de Bolsas',
                   'Taxa de Desistência Acumulada - TODA',
                   'Nome da Grande Área do Curso segundo a classificação CINE BRASIL']]
