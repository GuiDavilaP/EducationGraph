...
def filterData(nome_csv, selected, area=0, university="none"):
    #-------------------------------------Lê o arquivo CSV-----------------------------------------------
    df = pd.read_csv('arquivosCSV/' + nome_csv + '.csv', delimiter=";", encoding='cp1252')
    ...
    filtered_df['Percentual de Bolsas'] = (
        ...
        2).astype(float)
    filtered_df['Taxa de Desistência Acumulada'] = (filtered_df['Quantidade de Desistências'] / filtered_df[
        'Quantidade de Ingressantes no Curso'] * 100).round(2).astype(float)
